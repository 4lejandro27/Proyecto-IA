...
async def notify_business(lead_data: dict):
    """Envía notificación al negocio por email y Telegram."""
...
async def send_email_notification(lead_data: dict):
    ...
    if not settings.email_to or not settings.smtp_pass:
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        ...
        <p style="margin-top: 20px; color: #666; font-size: 12px;">
            Este lead fue generado automáticamente por ServiceBot.
        </p>
    </body>
    </html>
    """
...
async def send_telegram_notification(lead_data: dict):
    ...
    message = f"""
{emoji_urgencia.get(lead_data.get('urgencia', 'media'), '⚪')} *NUEVO LEAD - {lead_data.get('urgencia', 'MEDIA').upper()}*

👤 *Nombre:* {lead_data.get('nombre', 'N/A')}
📞 *Teléfono:* {lead_data.get('telefono', 'N/A')}
📧 *Email:* {lead_data.get('email', 'N/A')}
📍 *Dirección:* {lead_data.get('direccion', 'N/A')}, {lead_data.get('ciudad', 'N/A')}
